...
describe HumanClass do
	...
	context "on a human's turn" do
		...
		it "prompts them again after an invalid row number" do
			allow(human).to receive(:gets).and_return("4", "2")
			expect(human).to receive(:gets).twice
			human.get_row
		end

		it "asks the human player for the column they want to play on" do
			expect(human).to receive(:puts).with "And which column (1, 2 or 3)?"
			expect(human).to receive(:gets).and_return "2"
			human.get_square
		end

		it "prompts them again after an invalid column number" do
			allow(human).to receive(:gets).and_return("4", "2")
			expect(human).to receive(:gets).twice
			human.get_square
		end
